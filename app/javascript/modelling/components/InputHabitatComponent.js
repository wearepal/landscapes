...
export class InputHabitatComponent extends Component {
    ...
    async retrieveLandCoverData(bbox){
        ...
        const response = await fetch('https://landscapes.wearepal.ai/geoserver/wfs?' + new URLSearchParams({
        //const response = await fetch('http://localhost:8080/geoserver/wfs?' + new URLSearchParams({
    ...
    async worker(node, inputs, outputs) {
        ...
        let bbox = '-14782.045007023014,6596819.173144216,-7883.035964960155,6612578.505519601,EPSG:3857'

        let out = node.outputs;

        const activeHabitats = this.habitats.filter(habitat => out[habitat.mode].connections.length > 0)
            ...
            features.forEach(feature => {
                ...
                if(index != -1){
                    ...
                    if(geom.getType() === "Point"){
                        const [, x, y] = tileGrid.getTileCoordForCoordAndZ(geom.getCoordinates(), z)
                        activeHabitats[index].result.set(x, y, true)
                    }
                    else{
                        const featureTileRange = tileGrid.getTileRangeForExtentAndZ(geom.getExtent(), z)
