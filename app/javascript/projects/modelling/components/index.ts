...
import { ALC_Layer, nationalForestInventoryLayer, noneWoodlandTreesLayer } from "../non_woodland"
...
export function createDefaultComponents(saveMapLayer: SaveMapLayer, saveModel: SaveModel, getDatasets: getDatasets, extent: Extent, zoom: number, mask: boolean, maskLayer: string, maskCQL: string, permissions: ProjectPermissions, getCustomExpressions: getExpressionsType): BaseComponent[] {
    ...
    if (permissions.KewSamples) {
      ...
      restrictedComponents.push(new RasterComponent(projectProps, 'Wakehurst Soil Carbon', 'Kew', [
        { name: 'Carbon 0-15cm' , source: 'kew:predicted_carbon_015', area: 'ha', unit: 't' },
        { name: 'Carbon 0-30cm' , source: 'kew:predicted_carbon_030', area: 'ha', unit: 't' },
        { name: 'Carbon 15-30cm' , source: 'kew:predicted_carbon_1530', area: 'ha', unit: 't' },
        { name: 'Carbon 30-45cm' , source: 'kew:predicted_carbon_3045', area: 'ha', unit: 't' },
      ]))
    }
    ...
    const components : BaseComponent[] = [
      ...
      new VectorComponent('ALC Grade', ALC_Layer, projectProps),
